...
function readPalettes(): Record<Mode, Palette> {
    ...
    const pick = (b: string): Palette => ({
        ...
        swell: token(b, 'section-swell-indigo'),
    });
...
function buildTheme(mode: Mode, palette: Palette): string {
    const { background, foreground, swell } = palette;
   ...
   card with a section-swell gradient background, the site's inverted primary
...
${syntax}
    ...
    color-scheme: ${mode};
    /* One rounded card with a section-swell gradient (background to
       --section-swell-indigo to background), matching the home sections and the
       page-gradient wash. */
